...
class RotaryPositionalEmbedding(nn.Module):
    ...
    def forward(self, q, k):
        q_len = q.shape[-2]
        k_len = k.shape[-2]
        cache_len = max(q_len, k_len)
        cos, sin = self._update_cos_sin_cache(q, cache_len)
        q_cos = cos[:, :, :q_len, :]
        q_sin = sin[:, :, :q_len, :]
        k_cos = cos[:, :, :k_len, :]
        k_sin = sin[:, :, :k_len, :]
        return (
            (q * q_cos) + (self._rotate_half(q) * q_sin),
            (k * k_cos) + (self._rotate_half(k) * k_sin),
        )
...
class DependencyAwareLayer(nn.Module):
    ...
    def forward(self, hidden_states, sibling_embed, key_padding_mask=None):
        ...
        query_len = sibling_embed.size(1)
        hidden_len = hidden_states.size(1)

        if hidden_len == query_len:
            residual = hidden_states
        elif hidden_len > query_len:
            # During autoregressive decoding we query with only the newest step(s),
            # so align the residual connection with the matching tail positions.
            residual = hidden_states[:, -query_len:, :]
        else:
            raise ValueError(
                f"DependencyAwareLayer received hidden_states shorter than sibling_embed: "
                f"hidden_len={hidden_len}, query_len={query_len}"
            )

        if residual.shape != attn_out.shape:
            raise ValueError(
                "DependencyAwareLayer shape mismatch before residual add: "
                f"residual_shape={tuple(residual.shape)}, attn_out_shape={tuple(attn_out.shape)}"
            )
